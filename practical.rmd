...
- [MR-Base](http://www.mrbase.org/) which is built upon the [TwoSampleMR](https://github.com/MRCIEU/TwoSampleMR/) package, developed in Bristol
...
For this practical we will be using MR-Base because it integrates a database of GWAS summary data with functions for harmonising the data and analysing etc. It also can be used easily with several of the other packages.
...
Sometimes the MR-Base servers have problems, so all the data has been pre-extracted here in case it is not available from the servers
...
This was the subject of an analysis by [White et al 2016][1]. First we need to obtain instruments for urate levels. A quick way to do this is to see if a suitably powered urate GWAS is available in MR-Base, and extract the LD-clumped top hits
...
We have extracted 25 instruments for urate levels from this study.
...
**DISCUSS: What has happened here - why are only 24 SNPs being retained for MR analysis?**
